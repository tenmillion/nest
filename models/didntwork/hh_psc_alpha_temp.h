...
#ifndef HH_PSC_ALPHA_TEMP_H
#define HH_PSC_ALPHA_TEMP_H
...
namespace nest{
   ...
   */
  extern "C"
  int hh_psc_alpha_temp_dynamics(double, const double*, double*, void*);

 /* BeginDocumentation
Name: hh_psc_alpha_temp - Temperature-dependent Hodgkin Huxley neuron model.

Description:

  hh_psc_alpha_temp is an implementation of a spiking neuron using the Hodkin-Huxley formalism, modified to include the original temperature-dependent gating time constant.
...
Parameters: 
  ...
  phi		 double - Time constant of the gating variables, Q_10^((T-T_ex)/10)
  V_1		 double - Parameter of alpha_m
  V_2		 double - Parameter of beta_m
  V_3		 double - Parameter of alpha_h
  V_4		 double - Parameter of beta_h
  V_5		 double - Parameter of alpha_n
  V_6		 double - Parameter of beta_n
...
*/
  ...
  public:        
    ...
    hh_psc_alpha_temp();
    hh_psc_alpha_temp(const hh_psc_alpha_temp&);
    ~hh_psc_alpha_temp();
  ...
  private:
    ...
    friend int hh_psc_alpha_temp_dynamics(double, const double*, double*, void*);

    // The next two classes need to be friend to access the State_ class/member
    friend class RecordablesMap<hh_psc_alpha_temp>;
    friend class UniversalDataLogger<hh_psc_alpha_temp>;
  ...
  private:
    ...
    struct Parameters_ {
	  ...
	  double_t phi;		  //!< Time constant of gating variables
	  double_t V_1;		  //!< Parameter of alpha_m
	  double_t V_2;		  //!< Parameter of beta_m
	  double_t V_3;		  //!< Parameter of alpha_h
	  double_t V_4;		  //!< Parameter of beta_h
	  double_t V_5;		  //!< Parameter of alpha_n
	  double_t V_6;		  //!< Parameter of beta_n
  ...
  private:
     ...
     */
    struct Buffers_ {
      Buffers_(hh_psc_alpha_temp&);                   //!<Sets buffer pointers to 0
      Buffers_(const Buffers_&, hh_psc_alpha_temp&);  //!<Sets buffer pointers to 0

      //! Logger for all analog data
      UniversalDataLogger<hh_psc_alpha_temp> logger_;
    ...
    static RecordablesMap<hh_psc_alpha_temp> recordablesMap_;
  };

  inline
  port hh_psc_alpha_temp::check_connection(Connection& c, port receptor_type)
  {
    SpikeEvent e;
    e.set_sender(*this);
    c.check_event(e);
  ...
  inline
  port hh_psc_alpha_temp::connect_sender(SpikeEvent&, port receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  port hh_psc_alpha_temp::connect_sender(CurrentEvent&, port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  port hh_psc_alpha_temp::connect_sender(DataLoggingRequest& dlr, 
				    port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline
  void hh_psc_alpha_temp::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  inline
  void hh_psc_alpha_temp::set_status(const DictionaryDatum &d)
  {
